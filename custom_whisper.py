...
# custom whisper processor, uses CustomWhisperTokenizer if available
class CustomWhisperProcessor(WhisperProcessor):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if os.path.exists(tokenizer_config_file):
            with open(tokenizer_config_file, 'r', encoding='utf-8') as f:
                config = json.load(f)
                # check if custom tokenizer available
    ...
    # save processor with proper configurations
    def save_pretrained(self, save_directory, **kwargs):

        if isinstance(self.tokenizer, (CustomWhisperTokenizer)):
            # custom tokenizer will handle saving the custom config
            pass
        
        # parent's save_pretrained
